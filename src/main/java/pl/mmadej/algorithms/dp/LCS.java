...
public class LCS implements BiFunction<String, String, String> {
	...
	public String apply(String s1, String s2) {
		LcsPath[][] lcsPaths = initializeLcsPaths(s1.length(), s2.length());
		findLcs(s1, s2, lcsPaths);
		return resolveLcs(s1, s2, lcsPaths);
	}

	private void findLcs(String s1, String s2, LcsPath[][] lcsPaths) {
		...
		for (int i = 1; i <= s1.length(); i++) {
			for (int j = 1; j <= s2.length(); j++) {
				if (s1.charAt(i - 1) == s2.charAt(j - 1)) { //i-th letter of s1 is the same as j-th letter of s2 and is part of LCS for s1(0,i) & s2(0,j)
					...
					lcsPaths[i - 1][j - 1] = LcsPath.GO_WEST_NORTH;
				} else {
	...
	private String resolveLcs(String s1, String s2, LcsPath[][] lcsPaths) {
		int i = s1.length() - 1;
		int j = s2.length() - 1;
		String result = "";
		while (i >= 0 && j >= 0) {
			if (lcsPaths[i][j] == LcsPath.GO_WEST_NORTH) {
				result = s1.charAt(i--) + result;
				j--;
			} else if (lcsPaths[i][j] == LcsPath.GO_NORTH) {
				i--;
			} else {
				j--;
			}
	...
	private LcsPath[][] initializeLcsPaths(int rows, int columns) {
		LcsPath[][] lcsPaths = new LcsPath[rows][];
		for (int i = 0; i < rows; i++) {
			lcsPaths[i] = new LcsPath[columns];
		}
		return lcsPaths;
	}
	...
	//to reproduce lcs letter in matrix
	private enum LcsPath {
		GO_NORTH,
		GO_WEST,
		GO_WEST_NORTH
	}
